...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk){
    ...
    if(l->head == NULL || newblk_size < (l->head->blk->end - l->head->blk->start + 1)){
        newNode->next = l->head;
        l->head = newNode;
        return;
    }
...
void list_coalese_nodes(list_t *l){ 
    ...
    // First, sort the list by address to ensure adjacency
    // We'll create a temporary list sorted by address
    list_t *sorted = list_alloc();
    node_t *current = l->head;
    while(current != NULL){
        list_add_ascending_by_address(sorted, current->blk);
        current = current->next;
    }
    ...
    // Now, traverse the sorted list and merge adjacent blocks
    current = l->head;
    while(current != NULL && current->next != NULL){
        if(current->blk->end + 1 == current->next->blk->start){
            // Merge current and next
            block_t *merged_blk = malloc(sizeof(block_t));
            merged_blk->pid = 0; // Free block
            merged_blk->start = current->blk->start;
            merged_blk->end = current->next->blk->end;

            // Update current block
            current->blk->end = merged_blk->end;
